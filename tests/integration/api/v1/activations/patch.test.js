...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/72434e58-29f1-4d01-8b99-983946c2b58d",
        {
          method: "PATCH",
        },
      );
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      const activationTokenUser2 = await activation.create(user2.id);
      ...
      expect(responseBody).toEqual({
        ...
        action: `Verifique se seu usuario possui a feature "read:activation_token" e tente novamente.`,
        status_code: 403,
      });
    });
  });
